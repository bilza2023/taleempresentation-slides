<script>
  ...
  function updateImage(j, newValue) {
    // const newSp = [...$itemsStore[i].itemExtra.sp];
    // newSp[j].code = newValue;
    
    // updateStore(newSp);
  }
  
  // Helper function to update the store
  function updateStore(newSp) {
  ...
  {#each item.sp as sp, j}
    ...
    {#if (sp.type == 'img' || sp.type == 'image')}
      <div class="flex justify-center rounded-md w-8/12 mx-auto mb-4 mt-2 gap-1">
        <input 
          type="text" 
          class="bg-stone-900 rounded-md w-10/12 mx-auto" 
          value={sp.code}
          on:input={(e) => updateImage(j, e.target.value)}
        />
        <div class='w-2/12'>
          <VerticalBtns {moveUp} {moveDown} {del} {j}/>
        </div>
